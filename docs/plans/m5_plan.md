...
2. **Save corpus + load-compat test.** Check in a handful of saves
   ...
   *(Built with **real** saves, not synthetic ones: each era's file was
   generated by checking that milestone's commit out in a git worktree
   and running BasicPolicy for three game years. A hand-stripped save
   would only prove that serde ignores unknown fields, which isn't the
   thing at risk.)*
