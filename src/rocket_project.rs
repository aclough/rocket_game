...
pub struct RocketProject {
    ...
    /// Retired by the player — hidden from the Rockets pane, no longer
    /// counts toward what the company can bid on, and never auto-built.
    /// Kept in the vec so built hardware still resolves its design:
    /// inventory rockets, spacecraft in flight, and launch history all
    /// hold this project's id.
    #[serde(default)]
    pub retired: bool,
}
